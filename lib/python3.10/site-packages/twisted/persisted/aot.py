...
def getSource(ao):
    ...
    return indentify("chat = " + prettify(ao))
...
def unjellyFromSource(stringOrFile):
    """
    Pass me a string of code or a filename that defines an 'chat' variable (in
    ...
    if "chat" in ns:
        return unjellyFromAOT(ns["chat"])
    else:
        raise ValueError("%s needs to define an 'chat', it didn't!" % stringOrFile)
